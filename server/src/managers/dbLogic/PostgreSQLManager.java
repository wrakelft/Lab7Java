...
public class PostgreSQLManager implements DBmanager {
    ...
    public HashSet<Vehicle> getCollectionFromDB() {
        ...
        try (Connection connection = ConnectionFactory.getConnection();
            ...
            String query = "SELECT v.*, co.x, co.y " +
                    "FROM lab7.vehicle v " +
                    "JOIN lab7.coordinates co ON v.coordinates_id = co.id";
    ...
    public void writeCollectionToDB() {
        try (Connection connection = ConnectionFactory.getConnection()) {
            ...
            String getVehicleIdQuery = "SELECT id FROM lab7.vehicle";
    ...
    public long addElementToDB(Vehicle vehicle, Connection connection) {
        ...
        try {
            connection.setAutoCommit(false);
            String inCoordQuery = "INSERT INTO lab7.Coordinates (x, y) VALUES (?, ?) RETURNING id";
            ...
            String inVehicleQuery = "INSERT INTO lab7.vehicle (name, coordinates_id, creation_date, engine_power, vehicle_type, fuel_type)" +
                    "VALUES (?, ?, ?, ?, CAST(? AS lab7.vehicletype_enum), CAST(? AS lab7.fueltype_enum)) RETURNING id";
            ...
            String inCreatorQuery = "INSERT INTO lab7.creator (user_id, vehicle_id) VALUES (?, ?) ON CONFLICT (vehicle_id) DO NOTHING";
    ...
    public boolean removeVehicleById(long vehicleId) {
        try (Connection connection = ConnectionFactory.getConnection()) {
            ...
            String deleteVehicleQuery = "DELETE FROM lab7.vehicle WHERE id = ? AND id IN (SELECT vehicle_id FROM lab7.Creator WHERE user_id = ?)";
            String deleteCoordQuery = "DELETE FROM lab7.coordinates WHERE id = ?";
    ...
    public boolean removeVehiclesByList(List<Long> vehicleIds) {
        ...
        try (Connection connection = ConnectionFactory.getConnection()) {
            ...
            String selectVehiclesIdQuery = "SELECT vehicle_id FROM lab7.creator WHERE user_id = ?";
            ...
            if (!authVehicleIds.isEmpty()) {
                ...
                String deleteVehQuery = "DELETE FROM lab7.vehicle WHERE id IN (" + ids + ")";
                ...
                String deleteCoordQuery = "DELETE FROM lab7.coordinates WHERE id IN (" + ids + ")";
                PreparedStatement deleteCoordStatement = connection.prepareStatement(deleteCoordQuery);
    ...
    public List<Long> clearVehiclesForUser() {
        ...
        try (Connection connection = ConnectionFactory.getConnection()) {
            ...
            String deleteVehicleQuery = "DELETE FROM lab7.vehicle WHERE id IN (SELECT vehicle_id FROM lab7.Creator WHERE user_id = ?)";
    ...
    public boolean isVehicleOwnedByUser(long vehicleId) {
        try (Connection connection = ConnectionFactory.getConnection()) {
            ...
            String checkOwnerQuery = "SELECT COUNT(*) FROM lab7.Creator WHERE vehicle_id = ? AND user_id = ?";
    ...
    public boolean updateVehicle(Vehicle vehicle) {
        try (Connection connection = ConnectionFactory.getConnection()) {
            ...
            String updateCoordQuery = "UPDATE lab7.Coordinates SET x = ?, y = ? FROM lab7.vehicle WHERE vehicle.coordinates_id = Coordinates.id AND Vehicle.id = ?";
            ...
            String updateVehicleQuery = "UPDATE lab7.Vehicle SET name = ?, creation_date = ?, engine_power = ?, vehicle_type = CAST(? AS lab7.vehicletype_enum), fuel_type = CAST(? AS lab7.fueltype_enum) WHERE id = ?";
    ...
    public long authUser(String name, char[] passwd) {
        try (Connection connection = ConnectionFactory.getConnection()) {
            ...
            String selectUserQuery = "SELECT id, passwd_hash, passwd_salt FROM lab7.\"User\" WHERE name = ?";
    ...
    public long regUser(String name, char[] passwd) {
        try (Connection connection = ConnectionFactory.getConnection()) {
            ...
            String selectUserQuery = "SELECT COUNT(*) FROM lab7.\"User\" WHERE name = ?";
            PreparedStatement selectUserStatement = connection.prepareStatement(selectUserQuery);
            selectUserStatement.setString(1, name);
            ResultSet resultSet = selectUserStatement.executeQuery();
            ...
            String inUserQuery = "INSERT INTO lab7.\"User\" (name, passwd_hash, passwd_salt) VALUES (?, ?, ?)";
